...
public class vp_FPSCamera : vp_Component
{
	...
	int _rotateFingerId = -1;
	int _numFingersTouching = 0;
...
//#else
	...
	//	Debug.Log("touch down " + _numFingersTouching + " touching: " + m_Touching);
		if (m_Touching == false)
		{

			_rotateFingerId = touch.fingerId;
			m_Touching = true;
		}
...
//#endif
	...
	void OnTouchMoved(Touch touch)
	{
		...
		if (touch.fingerId == _rotateFingerId)
		{
	
			UpdateMouseLook(touch);
		}
	}
	...
	void OnTouchEnded(Touch touch)
	{
		...
		_numFingersTouching--;
//		Debug.Log("touch up " + _numFingersTouching);
		if (_rotateFingerId == touch.fingerId)
		{
			_rotateFingerId = -1;
			m_Touching = false;
			m_MouseMove = Vector3.zero;
		}
...
//		Vector3 attitudeDelta = Input.gyro.rotationRateUnbiased;
//		m_MouseMove.x = -attitudeDelta.y;
...
//		Debug.Log("touch delta: " + touchDelta + " pos : " + Input.touches[0].position);
		...
		float speed = .2f;
		m_MouseMove.x = touchDelta.x * speed;
		m_MouseMove.y = touchDelta.y  * speed;

//			#if UNITY_EDITOR
//			m_TouchStartPos = Input.mousePosition;
//			#else
//			m_TouchStartPos = InputManager.Instance.GetTouch(_rotateFingerId).position;
//			#endif
