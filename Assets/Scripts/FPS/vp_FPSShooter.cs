...
//				p.transform.Rotate(0, Random.Range(-ProjectileSpread, ProjectileSpread), 0);		// then rotate around y with user defined deviation
	...
	public string GetAmmoText()
	{
		...
		{
			return AmmoCount + " / " + GetAmmoInClip();
		}
	}
	...
	public bool Reload(int ammoCount)
	{
		...
		if (Time.time < m_NextAllowedReloadTime || (GetAmmoInClip() <= 0 && IsAmmoUnlimited == false))
			return true;

		int reloadAmmo = Mathf.Min(ammoCount - m_AmmoCount,TotalAmmo);

		m_NextAllowedReloadTime = Time.time + AmmoReloadTime;

		TotalAmmo -= reloadAmmo;
		m_AmmoCount += reloadAmmo;
